...
function renderPage(pageId: "connect" | "main"): void {
  ...
  if (pageId === "connect") {
    ...
    void (async () => {
      ...
      const quickSwitchTarget = sessionStorage.getItem("rylo:quick-switch-target");
      if (quickSwitchTarget !== null) {
        sessionStorage.removeItem("rylo:quick-switch-target");
...
log.info("Rylo client initialized");
