...
export function createLoginForm(opts: LoginFormOptions): LoginFormApi {
  ...
  function buildFormPanel(): HTMLDivElement {
    ...
    grad.setAttribute("id", "ry-grad-form");
    ...
    filter.setAttribute("id", "ry-glow-form");
    ...
    for (const [opacity, filterAttr] of [["0.4", "url(#ry-glow-form)"], [null, null]] as const) {
      ...
      t.setAttribute("fill", "url(#ry-grad-form)"); t.setAttribute("letter-spacing", "-4");
      if (opacity) { t.setAttribute("opacity", opacity); t.setAttribute("class", "ry-glow-layer"); }
      if (filterAttr) t.setAttribute("filter", filterAttr);
      t.textContent = "RY";
      logoSvg.appendChild(t);
    }
    const logoTitle = createElement("h1", {}, "Rylo");
