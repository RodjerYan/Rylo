...
// Expose debug info on window under __rylo namespace for DevTools console access
// Usage: JSON.stringify(__rylo.lkDebug(), null, 2)
const ryloNs = ((window as unknown as Record<string, unknown>).__rylo ??= {}) as Record<string, unknown>;
ryloNs.lkDebug = session.getSessionDebugInfo.bind(session);
