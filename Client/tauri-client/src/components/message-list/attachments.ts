...
/** Check if a URL points to the configured Rylo server. */
...
/** Report whether a URL targets the configured Rylo server host. */
...
const IDB_NAME = "rylo-image-cache";
...
export function fetchImageAsDataUrl(url: string): Promise<string | null> {
  ...
  const promise = (async (): Promise<string | null> => {
    ...
    // acceptInvalidCerts is required for self-hosted Rylo servers with self-signed
...
async function downloadFile(url: string, filename: string): Promise<void> {
  try {
    ...
    // Fetch file data — only accept invalid certs for the Rylo server
    const useInsecure = isServerUrl(url);
    const fetchOpts: RequestInit = useInsecure
      ? { danger: { acceptInvalidCerts: true, acceptInvalidHostnames: false } } as RequestInit
      : {};
