...
mod tests {
    ...
    #[test]
    fn allowed_key_rylo_prefix() {
        assert!(is_settings_key_allowed("rylo:profiles"));
        assert!(is_settings_key_allowed("rylo:settings:theme"));
        assert!(is_settings_key_allowed("rylo:recent-emoji"));
    }
    ...
    fn rejected_key_too_long() {
        let long_key = "rylo:".to_owned() + &"x".repeat(MAX_SETTINGS_KEY_LEN);
    ...
    fn rejected_key_partial_prefix_match() {
        // "rylo" without colon should not match "rylo:" prefix
        assert!(!is_settings_key_allowed("ryloNOCOLON"));
    }
