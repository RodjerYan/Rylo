...
describe("VoiceAudioTab camera preview", () => {
  beforeEach(() => {
    ...
    localStorage.setItem("rylo:settings:videoInputDevice", '"camera-1"');
...
describe("VoiceAudioTab UI structure", () => {
  ...
  it("restores saved input volume from preferences", () => {
    localStorage.setItem("rylo:settings:inputVolume", "75");
    stubNavigator();
    const ac = new AbortController();
    const tab = createVoiceAudioTab(ac.signal);
    const el = tab.build();
  ...
  it("restores saved output volume from preferences", () => {
    localStorage.setItem("rylo:settings:outputVolume", "60");
    stubNavigator();
    const ac = new AbortController();
    const tab = createVoiceAudioTab(ac.signal);
    const el = tab.build();
  ...
  it("stream quality select saves to preferences on change", () => {
    ...
    const saved = localStorage.getItem("rylo:settings:streamQuality");
  ...
  it("starts camera preview when a video device is saved", async () => {
    localStorage.setItem("rylo:settings:videoInputDevice", '"cam-1"');
    const cameraStream = {
      getTracks: () => [{ stop: vi.fn() }],
    } as unknown as MediaStream;
    const audioStream = {
  ...
  it("sensitivity threshold handle is positioned based on saved sensitivity", () => {
    localStorage.setItem("rylo:settings:voiceSensitivity", "75");
    stubNavigator();
    const ac = new AbortController();
    const tab = createVoiceAudioTab(ac.signal);
    const el = tab.build();
  ...
  it("restores saved device selections from localStorage", async () => {
    localStorage.setItem("rylo:settings:audioInputDevice", '"mic-2"');
    localStorage.setItem("rylo:settings:audioOutputDevice", '"spk-2"');
    stubNavigator([
      { kind: "audioinput", deviceId: "mic-2", label: "Mic 2" },
  ...
  it("restores saved stream quality selection", () => {
    localStorage.setItem("rylo:settings:streamQuality", '"low"');
    stubNavigator();
    const ac = new AbortController();
    const tab = createVoiceAudioTab(ac.signal);
    const el = tab.build();
