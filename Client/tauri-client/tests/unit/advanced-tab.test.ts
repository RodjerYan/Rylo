...
describe("AdvancedTab — Clear All Cache", () => {
  ...
  it("preserves rylo:profiles after Clear All Cache", async () => {
    localStorage.setItem("rylo:profiles", JSON.stringify([{ name: "Local", host: "localhost" }]));
    localStorage.setItem("rylo:settings:fontSize", "16");
    ...
    expect(localStorage.getItem("rylo:profiles")).not.toBeNull();
    // Other settings should be cleared
    expect(localStorage.getItem("rylo:settings:fontSize")).toBeNull();
    // sessionStorage should be cleared
    expect(sessionStorage.getItem("some-session-key")).toBeNull();
  });

  it("preserves credential keys after Clear All Cache", async () => {
    localStorage.setItem("rylo:credential:localhost", JSON.stringify({ user: "test" }));
    localStorage.setItem("rylo:settings:compactMode", "false");
    ...
    expect(localStorage.getItem("rylo:credential:localhost")).not.toBeNull();
    expect(localStorage.getItem("rylo:settings:compactMode")).toBeNull();
  });

  it("preserves active and custom theme keys after Clear All Cache", async () => {
    localStorage.setItem("rylo:theme:active", "custom-sunrise");
    localStorage.setItem("rylo:theme:custom:custom-sunrise", JSON.stringify({ name: "custom-sunrise" }));
    localStorage.setItem("rylo:settings:accentColor", '"#00c8ff"');
    ...
    expect(localStorage.getItem("rylo:theme:active")).toBe("custom-sunrise");
    expect(localStorage.getItem("rylo:theme:custom:custom-sunrise")).not.toBeNull();
    expect(localStorage.getItem("rylo:settings:accentColor")).toBeNull();
  });
...
describe("AdvancedTab — Toggles & Structure", () => {
  ...
  it("toggles Developer Mode on and persists to localStorage", () => {
    ...
    expect(toggle.classList.contains("on")).toBe(true);
    expect(toggle.getAttribute("aria-checked")).toBe("true");
    expect(localStorage.getItem("rylo:settings:developerMode")).toBe("true");

    // Toggle off again
    toggle.click();
    expect(toggle.classList.contains("on")).toBe(false);
    expect(localStorage.getItem("rylo:settings:developerMode")).toBe("false");
  });

  it("restores Developer Mode toggle state from localStorage", () => {
    localStorage.setItem("rylo:settings:developerMode", "true");
    const section = buildAdvancedTab(ac.signal);
    container.appendChild(section);
