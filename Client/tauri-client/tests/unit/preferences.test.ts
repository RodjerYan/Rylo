...
describe("preferences", () => {
  ...
  describe("STORAGE_PREFIX", () => {
    it("equals 'rylo:settings:'", () => {
      expect(STORAGE_PREFIX).toBe("rylo:settings:");
    });
  });
  ...
  describe("savePref", () => {
    ...
    it("dispatches an 'rylo:pref-change' CustomEvent with the key in detail", () => {
      const handler = vi.fn();
      window.addEventListener("rylo:pref-change", handler);
      ...
      window.removeEventListener("rylo:pref-change", handler);
    });
