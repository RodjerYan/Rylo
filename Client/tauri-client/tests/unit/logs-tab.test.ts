...
describe("LogsTab", () => {
  ...
  it("filter level persists via rylo:settings prefix", () => {
    mockGetLogBuffer.mockReturnValue([]);
    localStorage.clear();
    ...
    // Should use rylo:settings: prefix (normalized)
    expect(localStorage.getItem("rylo:settings:logs_filter_level")).toBe('"error"');
  });
  ...
  it("restores legacy unprefixed filter level and migrates it", () => {
    ...
    expect(localStorage.getItem("rylo:settings:logs_filter_level")).toBe('"warn"');
  });
  ...
  it("restores legacy unprefixed min level and migrates it", () => {
    ...
    expect(mockSetLogLevel).toHaveBeenCalledWith("error");
    expect(localStorage.getItem("rylo:settings:logs_min_level")).toBe('"error"');
  });
