...
describe("renderGenericLinkPreview", () => {
  ...
  it("allows previews for the configured Rylo server even on private hosts", async () => {
    setServerHost("LOCALHOST:8080");
    fetchMock.mockResolvedValue(mockHtmlResponse("<html><head><title>Rylo Local</title></head></html>"));
    ...
    await vi.waitFor(() => {
      expect(card.querySelector(".msg-embed-link-title")?.textContent).toBe("Rylo Local");
    });
  });
