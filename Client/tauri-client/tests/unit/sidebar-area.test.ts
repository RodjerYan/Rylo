...
function resetStores(): void {
  ...
  localStorage.removeItem("rylo:member-list-height");
  localStorage.removeItem("rylo:member-list-collapsed");
}
...
describe("SidebarArea", () => {
  ...
  describe("server header", () => {
    ...
    it("renders server icon with RY text", () => {
      const result = createSidebarArea(defaultOpts());
      container.appendChild(result.sidebarWrapper);

      const icon = container.querySelector(".server-icon-sm");
      expect(icon!.textContent).toBe("RY");
  ...
  describe("member list section", () => {
    it("restores member list height from localStorage", () => {
      localStorage.setItem("rylo:member-list-height", "200");
    ...
    it("restores collapsed state from localStorage", () => {
      localStorage.setItem("rylo:member-list-collapsed", "true");
    ...
    it("toggles member list collapse on header click", () => {
      ...
      expect(memberHeader.classList.contains("collapsed")).toBe(true);
      expect(localStorage.getItem("rylo:member-list-collapsed")).toBe("true");

      memberHeader.click();
      expect(memberHeader.classList.contains("collapsed")).toBe(false);
      expect(localStorage.getItem("rylo:member-list-collapsed")).toBe("false");

      cleanup(result);
    });

    it("sets height to auto when collapsed", () => {
      localStorage.setItem("rylo:member-list-height", "300");
    ...
    it("handles mousedown/mousemove/mouseup for drag-to-resize", () => {
      ...
      expect(localStorage.getItem("rylo:member-list-height")).toBeDefined();
    ...
    it("does not resize when not in drag mode", () => {
      ...
      expect(localStorage.getItem("rylo:member-list-height")).toBeNull();
