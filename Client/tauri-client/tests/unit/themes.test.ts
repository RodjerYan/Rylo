...
describe("themes", () => {
  ...
  it("exports a theme as JSON", () => {
    ...
    const parsed = JSON.parse(json) as RyloTheme;
  ...
  it("migrates the legacy settings theme key to the active theme key", () => {
    localStorage.setItem("rylo:settings:theme", JSON.stringify("midnight"));

    expect(getActiveThemeName()).toBe("midnight");
    expect(localStorage.getItem("rylo:theme:active")).toBe("midnight");
  });

  it("ignores an invalid active theme when a valid legacy theme exists", () => {
    localStorage.setItem("rylo:theme:active", "stale-theme");
    localStorage.setItem("rylo:settings:theme", JSON.stringify("midnight"));
...
describe("restoreTheme", () => {
  ...
  it("should apply saved theme name from localStorage", () => {
    localStorage.setItem("rylo:theme:active", "midnight");
    restoreTheme();
    expect(document.body.classList.contains("theme-midnight")).toBe(true);
  });

  it("should apply saved accent color on document", () => {
    localStorage.setItem("rylo:settings:accentColor", JSON.stringify("#00ff00"));
    restoreTheme();
  ...
  it("should reject accent color that is not valid hex", () => {
    localStorage.setItem("rylo:settings:accentColor", JSON.stringify("url(evil)"));
    restoreTheme();
    expect(document.documentElement.style.getPropertyValue("--accent")).toBe("");
  });

  it("should handle corrupted localStorage gracefully", () => {
    localStorage.setItem("rylo:settings:accentColor", "NOT VALID JSON {{{");
    // Should not throw
