...
test.describe("Connect Page", () => {
  ...
  test("form logo shows Rylo branding", async ({ page }) => {
    const logo = page.locator(".form-logo");
    await expect(logo).toBeVisible();

    // The logo contains an SVG with the "RY" text and an h1 with "Rylo"
    const logoSvg = logo.locator("svg.ry-logo");
    await expect(logoSvg).toBeVisible();

    const logoTitle = logo.locator("h1");
    await expect(logoTitle).toHaveText("Rylo");
  });
