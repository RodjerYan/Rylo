...
export const SERVER_URL = process.env.RYLO_SERVER_URL ?? "localhost:8443";
export const TEST_USER = process.env.RYLO_TEST_USER ?? "";
export const TEST_PASS = process.env.RYLO_TEST_PASS ?? "";
export const SKIP_SERVER = !!process.env.RYLO_SKIP_SERVER_TESTS;
...
export function logEnvironmentState(): void {
  ...
  if (!hasCredentials()) {
    console.log(
      "[native-e2e] WARNING: Set RYLO_TEST_USER and RYLO_TEST_PASS to enable authenticated tests",
    );
  }
}
 ...
 * Requires RYLO_TEST_USER and RYLO_TEST_PASS env vars.
 *
