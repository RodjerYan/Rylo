...
test.describe("Native App Server Connection", () => {
  test("health check via real Tauri HTTP plugin", async ({ nativePage }) => {
    ...
    // Skip if RYLO_SKIP_SERVER_TESTS is set.
    test.skip(
      !!process.env.RYLO_SKIP_SERVER_TESTS,
      "Skipped: RYLO_SKIP_SERVER_TESTS is set",
    );
  ...
  test("login attempt reaches real server", async ({ nativePage }) => {
    ...
    const serverUrl = process.env.RYLO_SERVER_URL ?? "localhost:8443";
    const username = process.env.RYLO_TEST_USER ?? "e2e-native-test";
    const password = process.env.RYLO_TEST_PASS ?? "e2e-native-test";
