...
func TestLoadDefaults(t *testing.T) {
	...
	tests := []struct {
		...
		{"Server.Name", cfg.Server.Name, "Rylo Server"},
...
func TestLoadEnvironmentVariableOverrides(t *testing.T) {
	// Environment variables with RYLO_ prefix should override config values.
	tmpDir := t.TempDir()
	cfgPath := filepath.Join(tmpDir, "config.yaml")

	t.Setenv("RYLO_SERVER_PORT", "7777")
	t.Setenv("RYLO_SERVER_NAME", "Env Server")
	t.Setenv("RYLO_DATABASE_PATH", "env/path.db")
	t.Setenv("RYLO_TLS_MODE", "manual")
...
func TestLoadEnvVarNoUnderscore(t *testing.T) {
	...
	// RYLO_PORT (no second underscore) — should not crash, just map to "port".
	tmpDir := t.TempDir()
	cfgPath := filepath.Join(tmpDir, "config.yaml")

	t.Setenv("RYLO_PORT", "1234")
...
func TestLoadEnvVarStorageDir(t *testing.T) {
	...
	t.Setenv("RYLO_UPLOAD_STORAGE_DIR", "/mnt/data/uploads")
