...
All requests to `/livekit/*` are reverse-proxied to the LiveKit server URL. The `/livekit` prefix is stripped before forwarding. This allows the client to connect to LiveKit through Rylo's HTTPS server, avoiding mixed-content blocks.
...
```json
{
  ...
  "platforms": {
    "windows-x86_64": {
      ...
      "url": "https://github.com/J3vb/Rylo/releases/download/v1.2.0/Rylo_1.2.0_x64-setup.nsis.zip"
    }
  }
}
```
