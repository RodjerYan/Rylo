...
Rylo server reads configuration from `config.yaml` in the working directory. On first run, if the file does not exist, a default `config.yaml` is created automatically.

Configuration is loaded in three layers (later layers override earlier ones):

1. **Built-in defaults** (compiled into the binary)
2. **YAML file** (`config.yaml`)
3. **Environment variables** (prefix: `RYLO_`)
...
| `server.name` | string | `"Rylo Server"` | Server display name (shown in `/api/v1/info` and admin panel) |
...
Every config key can be overridden via environment variables using the prefix `RYLO_`.

**Format:** `RYLO_<SECTION>_<KEY>`

| Environment Variable | Config Path |
|---------------------|-------------|
| `RYLO_SERVER_PORT` | `server.port` |
| `RYLO_SERVER_NAME` | `server.name` |
| `RYLO_SERVER_DATA_DIR` | `server.data_dir` |
| `RYLO_DATABASE_PATH` | `database.path` |
| `RYLO_TLS_MODE` | `tls.mode` |
| `RYLO_TLS_CERT_FILE` | `tls.cert_file` |
| `RYLO_TLS_DOMAIN` | `tls.domain` |
| `RYLO_UPLOAD_MAX_SIZE_MB` | `upload.max_size_mb` |
| `RYLO_UPLOAD_STORAGE_DIR` | `upload.storage_dir` |
| `RYLO_VOICE_LIVEKIT_API_KEY` | `voice.livekit_api_key` |
| `RYLO_VOICE_LIVEKIT_API_SECRET` | `voice.livekit_api_secret` |
| `RYLO_VOICE_LIVEKIT_URL` | `voice.livekit_url` |
| `RYLO_VOICE_NODE_IP` | `voice.node_ip` |
| `RYLO_VOICE_QUALITY` | `voice.quality` |
| `RYLO_GITHUB_TOKEN` | `github.token` |
